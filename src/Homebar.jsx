...
<div>{
    farmerdata.map((val,ind)=>{
        return(
            <div key={ind} onClick={()=>{setImgIndexdata(ind); clickHandler('null')}} className='d-flex ps-2 pt-2'><img src={val.farmerlogo} alt="" className='img-fluid rounded-circle cursor-pointer' style={{width:'50px',height:'50px',cursor:'pointer'}}/><p className='mt-2 ms-1 fs-5' style={{cursor:'pointer'}}>{val.farmername}</p></div>
        )
    }
    )
    }</div>
